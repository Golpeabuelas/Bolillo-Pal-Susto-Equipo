...
sesiones.post('/crearUsuario', (req, res) => {
    const nombre = req.body.nombre
    const correo = req.body.correo
    const password = req.body.password
    const permisos = req.body.permisos

    connection.query('INSERT INTO usuario (nombre, correo, password, permisos) VALUES (?, ?, ?, ?)', [nombre, correo, password, permisos], (error, response) => {
        if ( error ) {
